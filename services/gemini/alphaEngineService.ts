...
export async function processWatchlistItemForPlaybook(
    ...
    let basePlaybookItem: StrategyPlaybook = {
        ...
        strategySummary: dbData.rationale || '(?곗씠?곕쿋?댁뒪?먯꽌 ?쒓났???붿빟)',
...
export async function generatePlaybooksFromCandidates(
    ...
    const playbookSchema = {
        ...
        properties: {
            ...
            stockName: { type: Type.STRING },
            ticker: { type: Type.STRING },
            strategyName: { type: Type.STRING, enum: ['VCP (蹂?숈꽦 異뺤냼 ?⑦꽩)', 'Cup and Handle (컵앤핸들)', '플랫 베이스(Flat Base)', '상승 삼각형(Ascending Triangle)', 'AI 留ㅼ쭛遊??ъ갑 (湲곌?/?멸뎅???띾걣??', 'AI ?곹븳媛 ?ъ갑 ?붿쭊 (?쒖옣 二쇰룄二?', 'DB Signal', 'Hybrid Signal', '기타'] },
    ...
    const finalResponseSchema = {
        ...
        properties: {
            ...
            summary: { type: Type.STRING, description: "AI??寃곗젙 怨쇱젙怨?理쒖쥌 ?좏깮?????한국어?붿빟." }
        },
...
- In a '?섎씫?? or '怨좊??숈꽦 ?쇰???, you MUST **give highest priority to the 'AI 留ㅼ쭛遊??ъ갑 (湲곌?/?멸뎅???띾걣??' strategy if it is active**. Look for stocks that are resilient (rising on a down day) or being heavily accumulated by institutions/foreigners. Be extremely selective. It is acceptable to find zero signals if conviction is low.
- In a '?蹂?숈꽦 異붿꽭??, you can be more flexible. A stock matching 80% of a classic breakout strategy's criteria might be a valid candidate.`
...
- **TIME SENSITIVITY RULE (CRITICAL):**
    - **Closing Bell (종가 배팅):** ONLY suggest this if the Current Market Time is within 1 hour of market close (KR: 14:30-15:30, US: 15:00-16:00). If it is earlier, suggest 'SwingTrade' or 'DayTrade' instead.
...
- **OUTPUT:** Present all your findings and analysis as a comprehensive, unstructured text report in **KOREAN (한국어**. For each stock you select, clearly state **which strategy it matched** and **why**.
`;
...
- **strategyName**: This MUST be the name of the strategy you identified as a match in the CONTEXT. The available strategy names are: ['VCP (변동성 축소 패턴)', 'Cup and Handle (컵앤핸들)', '플랫 베이스(Flat Base)', '상승 삼각형(Ascending Triangle)', 'AI 매집봉 포착 (기관/외국인 수급)', 'AI 상한가 포착 돌진 (시장 주도주)', 'DB Signal', 'Hybrid Signal', '기타'].
...
- **strategyType**: 'SwingTrade' is the default unless the context or strategy name (e.g., ?곹븳媛) strongly suggests 'DayTrade'.
...
`;
    ...
    const fullPlaybooks = result.playbooks.map((p: any) => ({
        ...p,
        analysisChecklist: [],
        isUserRecommended: candidates.find(c => c.ticker === p.ticker)?.rationale === '사용자 관심종목',
    ...
    for (const priceResult of priceResults) {
        ...
        if (currentPrice === null) {
            localSignals.push({
                ...
                reason: `?ㅼ떆媛?媛寃??뺣낫瑜?媛?몄삤?????ㅽ뙣?섏뿬 ?좏샇 ?쒖꽦???щ?瑜??뺤씤?????놁뒿?덈떎. (${'error' in priceResult ? priceResult.error : 'Unknown error'})`,
                conflictingSignals: ['?곗씠???ㅻ쪟'], warning: '媛寃?議고쉶 ?ㅽ뙣',
            });
            continue;
        }
        ...
        if (currentPrice < stopLossPrice) {
            localSignals.push({
                type: 'NEUTRAL',
                ticker: p.ticker, stockName: p.stockName,
                reason: `?좏샇媛 臾댄슚?붾릺?덉뒿?덈떎. ?꾩옱媛(${currentPrice.toLocaleString()})媛 ?먯젅 湲곗?媛(${stopLossPrice.toLocaleString()})瑜??섑뼢 ?댄깉?덉뒿?덈떎.`,
                conflictingSignals: ['?먯젅 湲곗? ?꾨떖'], warning: '?뚮젅?대턿 臾댄슚',
            });
            continue;
        }
        ...
        if (currentPrice >= entryPrice) {
            localSignals.push({
                ...
                tradingPlan: {
                    ...
                    targets: [p.keyLevels.target || `?먮룞 怨꾩궛`],
                    positionSizing: `(AI 異붿쿇 鍮꾩쨷)`,
...
**3. Execution Strength (泥닿껐媛뺣룄 & ?멸?李?:**
...
async function generatePlaybooksForBatch(
    ...
    const playbookSchema = {
        ...
        properties: {
            ...
            stockName: { type: Type.STRING },
            ticker: { type: Type.STRING },
            strategyName: { type: Type.STRING, enum: ['VCP (蹂?숈꽦 異뺤냼 ?⑦꽩)', 'Cup and Handle (컵앤핸들)', '플랫 베이스(Flat Base)', '상승 삼각형(Ascending Triangle)', 'AI 留ㅼ쭛遊??ъ갑 (湲곌?/?멸뎅???띾걣??', 'AI ?곹븳媛 ?ъ갑 ?붿쭊 (?쒖옣 二쇰룄二?', 'DB Signal', 'Hybrid Signal', '기타'] },
            strategySummary: { type: Type.STRING },
            aiConfidence: { type: Type.NUMBER },
            keyLevels: { type: Type.OBJECT, properties: { entry: { type: Type.STRING }, stopLoss: { type: Type.STRING }, target: { type: Type.STRING } }, required: ['entry', 'stopLoss', 'target'] },
            strategyType: { type: Type.STRING, enum: ['DayTrade', 'SwingTrade', 'LongTerm'] },
...
**CRITICAL RULES:**
- All text MUST be in **KOREAN (한국어)**. Do NOT use English for explanations.
- Translate all technical terms to Korean context where appropriate (e.g., Pivot Point -> 피벗 포인트, Volatility -> 변동성).
${ANTI_HALLUCINATION_RULE}
...
Respond ONLY with a valid JSON array of objects matching the provided schema.`;
    ...
    const getSourceFromRationale = (ticker: string, rationale: string): string[] => {
        ...
        if (r.includes('종가배팅')) return ['bfl'];
        if (r.includes('재료 레이더')) return ['material'];
        if (r.includes('패턴 스크리너')) return ['pattern'];
        if (r.includes('수급 독수리')) return ['supply_eagle'];
        if (r.includes('Eagle Eye') || r.includes('Volume Spike')) return ['scanner'];
        if (r === '사용자 관심종목') return ['user'];
...
async function fetchAlphaPreviewLatest(ticker: string): Promise<any | null> {
    ...
    try {
        ...
        return {
            ...
            market: data.market,
            rationale: plan.planRationale || data.signal_type || '분석 데이터',

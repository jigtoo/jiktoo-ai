...
import {
    BellIcon, PlusIcon, InfoIcon,
    BrainIcon, StrategistIcon, VerdictIcon, HomeIcon
} from './icons';
...
export const ExecutiveDecisionBriefing: React.FC<{
    ...
    onGoHome?: () => void;
    onSubscribeWatcher?: (ticker: string, stockName: string) => void;
}> = ({ result, onOpenFormForAnalysis, onGoHome, onSubscribeWatcher }) => {
            ...
            <header className="flex flex-col sm:flex-row justify-between sm:items-start gap-3 mb-4">
                ...
                <div className="flex items-center gap-2 self-end sm:self-center">
                    <div className="relative group flex items-center gap-1">
                        <button
                            onClick={() => onSubscribeWatcher?.(ticker, stockName)}
                            className="flex items-center gap-2 px-3 py-1.5 text-sm font-semibold rounded-md transition-colors bg-gray-600 text-white hover:bg-gray-500"
                        >
